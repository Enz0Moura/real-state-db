...
int example() {
    ...
    try {
        ...
        insert_contrato_aluguel(connection, id_inquilino2, imovel->getAttribute("id"), BaseTable::generateUUID(), 11000, "2026-06-12", "2020-06-12");

        std::vector<std::shared_ptr<BaseTable>> inquilinos = BaseTable::fetchAll(connection, "inquilinos");

        for (const auto& record : inquilinos) {
            auto inquilino_it = std::dynamic_pointer_cast<Inquilino>(record);
            if (inquilino_it) {
                std::cout << "Inquilino: " << inquilino_it->getAttribute("nome")
                          << " - CPF: " << inquilino_it->getAttribute("cpf") << std::endl;
            }
        }
